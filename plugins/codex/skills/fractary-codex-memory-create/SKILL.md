...
5. **ALWAYS ask the user interactively** when --template or --context is not provided, to guide the user through creation
...
Otherwise, ask the user to select a type by presenting the available options:
...
If `--context` is NOT provided, ask the user to describe the knowledge they want to capture — what happened, what they learned, and why it matters.

If `--title` is NOT provided, derive a concise title from the context or ask the user for one.
...
**Sequence number:** Scan the type directory to find the next available sequence number. Search for files matching `.fractary/codex/memory/{type}/MEM-{PREFIX}-*.md`, count existing files and increment by 1. Pad to 3 digits (e.g., `001`, `002`, `042`).
...
Write the populated memory file to `.fractary/codex/memory/{memory_type}/MEM-{PREFIX}-{SEQ}-{slug}.md`.
...
Next steps:
  ...
  - Run the fractary-codex-memory-audit skill to validate memories periodically
```
